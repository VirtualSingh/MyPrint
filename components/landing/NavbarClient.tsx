...
import { NavbarUserMenu } from '@/components/landing/NavbarUserMenu'
  ...
  return (
    <nav className="sticky top-0 z-50 bg-white border-b border-border-subtle">
      <div className="max-w-container mx-auto px-6 h-16 flex items-center justify-between">
        {/* Logo */}
        <Link href="/" className="flex items-center gap-2 text-brand font-semibold text-base">
        ...
        <div className="hidden md:flex items-center gap-3">
          {user ? (
            <NavbarUserMenu name={user.name} email={user.email} avatarUrl={user.avatarUrl} />
          ) : (
            <Link
              href="/login"
